...
march: do istep = 1, dnstep
    ...
    loop_weight: do i = 1, nx
        ...
        ! calculate weighting factor W-4 (4.14).
        avl (:) = dabs(vr(:))   ! note the inversion of l/r.
        avr (:) = dabs(vl(:))
        do j = 1, size(avl)
            if (avl(j) > avr(j)) then
                eta = avl(j)/(avr(j)+1.d-200) - 1.d0
                avl(j) = (1.d0 + eta/tau) * avr(j)
            else
                eta = avr(j)/(avl(j)+1.d-200) - 1.d0
                avr(j) = (1.d0 + eta/tau) * avl(j)
            end if
        end do

...
march: do istep = 1, dnstep
    ...
    loop_weight: do i = 1, nx
        ...
        ! calculate weighting factor using W-2 (4.4) with input different 
        ! to W-1.
        avl (:) = dabs((midu(:,i  ) - up  (:,i))/(midxs(i  )-sidxs(i)))
        avr (:) = dabs((up  (:,i+1) - midu(:,i))/(sidxs(i+1)-midxs(i)))

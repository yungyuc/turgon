...
The basic concepts behind the CESE method are\cite{chang_method_1995}:
%
\begin{enumerate}
%
\item Flux conservation is enforced in both space and time.
%
\item Space and time are treat homogeneously.
%
\item The mesh value of both the dependent variables and their derivatives are
both solved in the schemes.
%
\item The utilized discrete variables are local in stencil.
%
\item The conservation elements and solution elements are chosen to result in
compact stencil.
%
\item Space-time invariants are used to minimize numerical dissipation.
%
\item Characteristics-based methods are excluded.
%
\item Ad hoc treatments are avoided as much as possible.
%
\end{enumerate}
%
...
scheme\cite{chang_method_1995}, $c$ scheme, and $c$-$\tau$/$c$-$\tau^*$
scheme\cite{chang_multi-dimensional_2003}.  Methods for weighted averaging,
such as $\alpha$ scheme\cite{chang_method_1995}, Scheme-I,
Scheme-II\cite{chang_courant_2002}, W-1 scheme, and W-2
scheme\cite{chang_multi-dimensional_2003} are also stated.  The note is
